...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202407141716-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  LICENSE.md
# @@ReadME           :  zz-tftpd.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Sunday, Jul 14, 2024 17:16 EDT
# @@File             :  zz-tftpd.sh
# @@Description      :
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TMP_DIR="/tmp/tftpd"
RUN_DIR="/run/tftpd"       # set scripts pid dir
LOG_DIR="/data/logs/tftpd" # set log directory
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the working dir
WORK_DIR="/data/tftpd" # set working directory
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
SERVICE_USER="tftpd"  # execute command as another user
SERVICE_GROUP="tftpd" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
EXEC_CMD_BIN='in.tftpd'                         # command to execute
EXEC_CMD_ARGS='-l -R 4096:32767 -s /data/tftpd' # command arguments
...
[ -f "$CONF_DIR/env/tftpd.script.sh" ] && . "$CONF_DIR/env/tftpd.script.sh" # Generated by my dockermgr script
[ -f "$CONF_DIR/env/tftpd.sh" ] && . "$CONF_DIR/env/tftpd.sh"               # Overwrite the variabes
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  [ -d "/data/tftpd" ] || mkdir -p "/data/tftpd"
  [ -d "/var/tftpboot" ] && rm -Rf "/var/tftpboot"
  [ -d "/data/tftpd/pxelinux.cfg/default" ] || mkdir -p "/data/tftpd/pxelinux.cfg/default"
  # define actions
  ln -sf "/data/tftpd" "/var/tftpboot"
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$TFTPD_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$TFTPD_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$TFTPD_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$TFTPD_USER_PASS_WORD}"             #
